...
class FBXTreeParser {
  ...
  parseScene(deformers, geometryMap, materialMap) {
    ...
    let animations = [];
    try {
      animations = new AnimationParser().parse();
    } catch (err) {
      console.warn("FBX animations skipped", err);
      animations = [];
    }
        ...
        children.forEach(function(child, i) {
          if (curveNodesMap.has(child.ID)) {
            ...
            if (curveNode.curves.x !== void 0 || curveNode.curves.y !== void 0 || curveNode.curves.z !== void 0) {
              ...
              if (layerCurveNodes[i] === void 0) {
                try {
                  const deformerRel = connections.get(child.ID);
                  const deformerID = deformerRel.parents.filter(function(parent) {
                    return parent.relationship !== void 0;
                  })[0].ID;
                  ...
                  const node = {
                    modelName: rawModel && rawModel.attrName ? PropertyBinding.sanitizeNodeName(rawModel.attrName) : "",
                    morphName: fbxTree.Objects.Deformer[deformerID] ? fbxTree.Objects.Deformer[deformerID].attrName : ""
                  };
                  layerCurveNodes[i] = node;
                } catch {
                  return;
